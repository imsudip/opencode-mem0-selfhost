...
Also set `metadata.pinned = true` so other tools (e.g. `/mem0-dream`) can
recognize it without parsing the text:

```python
update_memory(
    id=<selected_id>,
    metadata={**original_metadata, "pinned": True},
)
```

The self-host server is synchronous — the response from `update_memory` already
contains the updated memory. No `get_event_status` polling needed (that tool
returns `UNSUPPORTED` on self-host).
...
1. Call `add_memory` with:
   ...
   - `infer=False`
2. The response is the written memory directly. Extract the `id` from it and confirm.
...
2. Remove the pin marker from the text:
   ...
   ```
3. Call `update_memory` with both:
   - `text=unpinned_text`
   - `metadata={**original_metadata, "pinned": False}`
