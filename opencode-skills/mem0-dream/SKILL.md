...
Call `get_memories` to retrieve every memory for the active project. Paginate
because the self-host server returns at most `page_size` per call:
...
1. `add_memory` with:
   ...
   - `app_id=<active_project_id>` (the plugin stores it as `metadata.app_id`)
   - `metadata={"type": "<original type>", "branch": "<active_branch>", "confidence": <higher of the two original scores>, "source": "mem0-dream"}`
   - `infer=False`
2. `delete_memory(<id1>)`
3. `delete_memory(<id2>)`

Order matters: write the new memory first, then delete the originals. If the
write fails, leave the originals in place and surface the error.
