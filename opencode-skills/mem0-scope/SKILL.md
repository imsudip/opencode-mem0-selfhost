...
- `project` (default) — this repo only. Filters by `user_id` + `app_id` (which
  the plugin stores in `metadata.app_id` because the self-host REST API does
  not have a first-class `app_id` field).
