...
Store a fact or learning directly into the self-host mem0 server.
...
The self-host server writes synchronously, so the `add_memory` response already
contains the memory directly (it may be a single object or a list). Extract the
`id` field — that is the memory ID.

Do NOT call `get_event_status` — on self-host mem0 it is a no-op that returns
`{status: "UNSUPPORTED", ...}`. There is no async write to wait for.
