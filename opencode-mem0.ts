...
// (add / search / manage) via a self-host Mem0 REST server, wired through
// OpenCode plugin hooks. Memory operations are exposed as native OpenCode
...
function extractMemories(res: Memory[] | { results?: Memory[] } | undefined): Array<{ memory: string; id: string }> {
  const arr = Array.isArray(res) ? res : res?.results;
  if (!Array.isArray(arr)) return [];
  return arr
    .filter((m): m is Memory => Boolean(m))
    .map((m) => ({memory: m.memory ?? "", id: m.id ?? ""}));
}
...
const Mem0Plugin: Plugin = async (ctx) => {
    ...
    try {
      await client.app.log({
        body: {
          ...
          level: "error",
          message:
            "MEM0_API_KEY environment variable not set. Set it to the X-API-Key value accepted by your self-host Mem0 server.",
        },
      });
    } catch {
    }
  ...
  const mem0 = new SelfHostMemoryClient({apiKey, defaultUserId: userId, defaultAppId: appId});
...
Use the mem0 memory tools (add_memory, search_memories, get_memories, get_memory, update_memory, delete_memory, delete_all_memories, delete_entities, list_entities, get_event_status) as instructed by the skill. This is a self-host Mem0 instance: writes are synchronous (no event_id to poll), and get_event_status is a no-op that returns {status: "UNSUPPORTED"}.
  ...
  return {
    ...
    tool: {
      ...
      get_event_status: tool({
        description: "Compatibility helper. Self-host Mem0 REST writes are synchronous and do not expose event status. Always returns {status: 'UNSUPPORTED', message: '...'}.",
        args: {
          event_id: tool.schema.string().describe("Ignored self-host compatibility event ID"),
        },
        async execute(args) {
          ...
          return JSON.stringify({
            ...
            message: "Self-host Mem0 REST writes are synchronous and do not expose event status. The add_memory response already contains the memory ID.",
          });
        }
      }),
    },
  ...
  async function chatMessageHook(input: any, output: any) {
    ...
    if (!initialized) {
      ...
      try {
        ...
        const c = (all as { count?: number })?.count;
        memoryCount = typeof c === "number" ? c : 0;
